...
def log_likelihood(ni, Ni, D, N):
    ...
    if NC < 0:
        likelihood = - 70 #1e-70 è abbastanza zero
    else:
        L1 = stat.multivariate_hypergeom.pmf(x=ni, m=int_Ni, n=sum(ni))
        L2 = stat.multivariate_hypergeom.pmf(x=ni, m=int_Ni+1, n=sum(ni))
        L = (L1+L2)/2
        if L < 1e-70: #per vevitare problemi nel log
            likelihood = - 70
        else:
            likelihood =  np.log(L)
...
def samplig(ni, x, D, bound, step, N):
    ...
    while True:
        ...
        new_sample[D] = log_likelihood(ni, new_sample[:D], D, N)
...
def nested_samplig(ni, N, D, bound, tau=1e-6, verbose=False):
    """
    ...
    prior_sample = np.zeros((N, D), dtype=int)
    for i in range(D):
    ...
    for i in range(N):
        grid[i, D] = log_likelihood(ni, prior_sample[i,:], D, N)
...
if __name__ == "__main__":
    ...
    NS = nested_samplig(ni, N, D, bound, verbose=True)
